...
def main():
    ...
    os.mkdir(workingdir + 'runs/LargeVesc2/VerticleAttempt/init_{:.3f}_{:.3f}_{:d}_{:d}_{:.1f}'.format(c0,c_c0,seed,eps_mc,sigma_c))
    outfile=workingdir + 'runs/LargeVesc2/VerticleAttempt/init_{:.3f}_{:.3f}_{:d}_{:d}_{:.1f}/'.format(c0,c_c0,seed,eps_mc,sigma_c) + 'particles.in'
    f=open(outfile,'w')
    ...
    for ii in range(Ndumb):
        f.write(str(Nout+Nmem+Nin+ii+1) + '\t' + '4' + '\t' + str(0) + '\t' + str(0) + '\t' + str(zMAX+ii*(sigma_c)-0.5*sigma_c-0.1) + '\t1\t1\t0\t0\t0\t0\t' + str(Nout+Nmem+Nin+1) + '\n')
    f.write('\n')
    f.close()
    ...
    outfile2 = workingdir + 'runs/LargeVesc2/VerticleAttempt/init_{:.3f}_{:.3f}_{:d}_{:d}_{:.1f}/in.local'.format(c0,c_c0,seed,eps_mc,sigma_c)
    fo=open(outfile2,'w')
