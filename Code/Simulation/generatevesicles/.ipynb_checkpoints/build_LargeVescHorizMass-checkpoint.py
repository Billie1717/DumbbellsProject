...
def main():
    ...
    dt = 0.01							                # time step
    thermo_interval = 8000                              # interval for measuring thermo quants.
    ...
    ################ Spherical Membrane ################
    Nmem = 9722
    workingdir = '//Users/billiemeadowcroft/Dropbox/PHD_data/Dumbbells/Simulations/'
    fname_in = workingdir +'relaxedvesiclefiles/%d_relaxed_vesicle.xyz'%(Nmem) 			# read in positions of membrane particles
    ...
    #os.mkdir(workingdir + 'runs/Horizontal/init_{:.3f}_{:.3f}_{:d}_{:d}_{:.1f}'.format(c0,c_c0,seed,eps_mc,sigma_c))
    outfile=workingdir + 'runs/Horizontal/init_{:.3f}_{:.3f}_{:d}_{:d}_{:.1f}/'.format(c0,c_c0,seed,eps_mc,sigma_c) + 'particles.in'
    f=open(outfile,'w')
    ...
    for ii in range(Ndumb):
        f.write(str(Nout+Nmem+Nin+ii+1) + '\t' + '4' + '\t' + str(ii*sigma_c-0.5*sigma_c) + '\t' + str(0) + '\t' + str(zMAX-sigma_c*0.5+0.1) + '\t1\t1\t0\t0\t0\t0\t' + str(Nout+Nmem+Nin+1) + '\n')
    f.write('\n')
    f.close()
    ...
    outfile2 = workingdir + 'runs/Horizontal/init_{:.3f}_{:.3f}_{:d}_{:d}_{:.1f}/in.local'.format(c0,c_c0,seed,eps_mc,sigma_c)
    fo=open(outfile2,'w')
    ...
    fo.write('\n')
    fo.write('set group rest mass 1.0\n')
    fo.write('\n')
    fo.write('set group cargo mass 30.0\n')
    fo.write('\n')
    ...
    fo.write('\n')
    fo.write('################ dump ################\n')
    fo.write('dump\tcoords\tall\tcustom '+str(int(dump_interval/40))+' output.xyz id mol type x y z mux muy muz c_stress[1] c_stress[2] c_stress[3] c_stress[4] c_stress[5] c_stress[6] v_patom c_cluster_mem c_cluster_cyto\n')
