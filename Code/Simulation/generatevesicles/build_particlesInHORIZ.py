...
'''
sys.argv [run_time] [init conc of solute] [ratio between concs- bigger than 1 = more on inside] [ratio between radius] [cores] [seed]
'''
...
def main():
    ...
    os.mkdir(workingdir + 'runs/init_{:.3f}_{:.3f}_{:d}'.format(c0,c_c0,seed))
    outfile=workingdir + 'runs/init_{:.3f}_{:.3f}_{:d}/'.format(c0,c_c0,seed) + 'particles.in'
    f=open(outfile,'w')
    ...
    for ii in range(Ndumb):
        f.write(str(Nout+Nmem+Nin+ii+1) + '\t' + '4' + '\t' + str(6*ii) + '\t' + str(0) + '\t' + str(zMAX+3.1) + '\t1\t1\t0\t0\t0\t0\t' + str(Nout+Nmem+Nin+1) + '\n')
    f.write('\n')
    f.close()
    ...
    outfile2 = workingdir+'runs/init_{:.3f}_{:.3f}_{:d}/in.local'.format(c0,c_c0,seed)
    fo=open(outfile2,'w')
